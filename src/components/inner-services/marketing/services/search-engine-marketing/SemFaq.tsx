...
const { faq } = SEARCH_ENGINE_MARKETING_PAGE;

export function SemFaq() {
  const baseId = useId();
    ...
    <section className="w-full border-t border-[var(--border-subtle)] bg-[var(--bg-elev)] py-10 sm:py-11">
      <ContentContainer>
        <h2 className="mb-5 max-w-3xl text-balance font-sans text-4xl font-semibold leading-[1.08] tracking-[-0.02em] text-text sm:text-4xl md:text-5xl md:font-bold">
          The <span className="text-[var(--red)]">SEM client</span> ask before starting
        </h2>
              ...
              <div key={q} className={isLast ? "" : "border-b border-[var(--border-subtle)]"}>
                ...
                >
                  <div className="min-h-0 overflow-hidden">
                    <p className="border-t border-[var(--border-subtle)] px-4 pb-4 pt-3 text-sm leading-relaxed text-[var(--text-muted)]">
                      {a}
                    </p>
                  </div>
                </div>
