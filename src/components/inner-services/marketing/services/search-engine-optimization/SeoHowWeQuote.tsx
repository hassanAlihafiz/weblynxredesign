...
const { howWeQuote } = SEARCH_ENGINE_OPTIMIZATION_PAGE;

export function SeoHowWeQuote() {
  return (
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_1.4fr] lg:items-start lg:gap-6">
          <div>
            <h2 className="mb-5 max-w-3xl text-balance font-sans text-4xl font-semibold leading-[1.08] tracking-[-0.02em] text-text sm:text-4xl md:text-5xl md:font-bold">
              Every business is different <span className="text-[var(--red)]">So is every quote</span>
            </h2>
          </div>
          <div>
            <p className="mb-5 max-w-3xl text-balance text-lg leading-relaxed text-[var(--text-muted)]">{howWeQuote.intro}</p>
            <div className="grid gap-2.5 sm:grid-cols-3">
              {howWeQuote.steps.map(({ num, title, body }) => (
                ...
                >
                  <div className="mb-1 font-sans text-[2rem] font-bold leading-none text-[var(--red)]">{num}</div>
