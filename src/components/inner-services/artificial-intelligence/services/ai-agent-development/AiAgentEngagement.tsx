...
const { engagementModels } = AI_AGENT_DEVELOPMENT_PAGE;

export function AiAgentEngagement() {
  return (
    <section className="w-full border-t border-[var(--border)] py-11 sm:py-12">
      <ContentContainer>
        ...
        <p className="mb-5 max-w-xl text-lg leading-relaxed text-[var(--text-muted)]">{engagementModels.description}</p>
        ...
        <div className="grid gap-3 md:grid-cols-3 md:items-stretch">
          {engagementModels.tiers.map((tier) => (
            <article
              ...
              className={
                tier.highlighted
                  ? "relative rounded-[var(--border-radius-md)] border-2 border-[var(--red-glow)] bg-gradient-to-b from-[rgba(230,57,70,0.05)] to-transparent p-5"
            ...
            >
              ...
              <p className="mb-1.5 text-sm uppercase tracking-wide text-[var(--text-muted)]">{tier.code}</p>
