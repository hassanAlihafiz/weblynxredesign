...
import { GenAiHeroGraphic } from "./GenAiHeroGraphic";

const { hero } = GENERATIVE_AI_PAGE;
        ...
        <ContentContainer className="relative z-[2]">
          <div className="grid items-center gap-8 lg:grid-cols-[1.3fr_1fr] lg:gap-8">
            <div className="min-w-0">
              <h1 className="mb-3.5 max-w-3xl text-balance font-sans text-5xl font-bold leading-[1.05] tracking-[-0.03em] leading-tight text-[var(--text)] md:text-6xl">
                AI that <span className="text-[var(--red)]">creates</span>, not just answers
              </h1>
              ...
              <div className="mt-6 flex flex-wrap gap-6 sm:gap-8">
                {hero.stats.map(({ value, label }) => (
                  <div key={label}>
                    <p className="font-sans text-4xl font-bold leading-tight text-[var(--red)]">{value}</p> 
                    <p className="mt-0.5 text-sm font-medium leading-tight uppercase tracking-wide text-[var(--text-muted)]">
                      {label}
                    </p>
                  </div>
                ))}
            ...
            <div className="relative flex h-[280px] items-center justify-center">
              <div
                className="absolute size-[240px] rounded-full bg-[radial-gradient(circle,var(--red-glow),transparent_70%)] blur-[20px]"
                aria-hidden
              />
              <GenAiHeroGraphic />
            </div>
