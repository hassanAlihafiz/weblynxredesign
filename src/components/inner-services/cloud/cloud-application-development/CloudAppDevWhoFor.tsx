...
const { whoFor } = CLOUD_APPLICATION_DEVELOPMENT_PAGE;

export function CloudAppDevWhoFor() {
  return (
    <section className="w-full border-t border-[var(--border)] bg-[var(--bg-elev)] py-9 sm:py-11">
      <ContentContainer>
        <h2 className="mb-5 max-w-3xl text-balance font-sans text-5xl font-bold leading-[1.05] tracking-[-0.03em] text-[var(--text)] md:text-6xl">
          You probably need <span className="text-[var(--red)]">cloud app dev</span> if...
        </h2>
        ...
        <div className="grid gap-2.5 sm:grid-cols-2">
          {whoFor.bullets.map((text) => (
            ...
            >
              ...
              <span className="text-sm leading-relaxed text-[var(--text)] sm:text-sm sm:leading-relaxed">{text}</span>
            </div>
          ))}
        </div>
