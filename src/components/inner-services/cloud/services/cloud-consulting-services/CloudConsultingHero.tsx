import { IconArrowDown, IconCloudNetwork } from "@tabler/icons-react";
...
import { CloudConsultingHeroGraphic } from "./CloudConsultingHeroGraphic";

const { hero } = CLOUD_CONSULTING_SERVICES_PAGE;

export function CloudConsultingHero() {
  return (
    <>
        ...
        <ContentContainer className="relative z-[2]">
          <div className="grid items-center gap-8 lg:grid-cols-[1.3fr_1fr] lg:gap-8">
            <div className="min-w-0">
              ...
              <h1 className="mb-3.5 max-w-3xl text-balance text-5xl font-bold leading-[1.05] tracking-[-0.03em] text-[var(--text)] md:text-6xl">
                Cloud done <span className="text-[var(--red)]">right</span> not just done
              </h1>

              <p className="mb-5 max-w-md text-lg leading-relaxed text-[var(--text-muted)] sm:text-sm sm:leading-relaxed">
                {hero.description}
              </p>
              ...
              <div className="mt-6 flex flex-wrap gap-6 sm:gap-8">
                {hero.stats.map(({ value, label }) => (
                  <div key={label}>
                    <p className="font-sans text-lg font-semibold leading-none text-[var(--red)]">{value}</p>
                    <p className="mt-0.5 text-lg font-medium uppercase tracking-[0.06em] text-[var(--text-muted)]">
                      {label}
                    </p>
                  </div>
                ))}
