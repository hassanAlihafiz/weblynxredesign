...
import { CLOUD_CONSULTING_SERVICES_PAGE, type CloudConsultingCapabilityIconId } from "@/data/site";

const capabilityIcons: Record<CloudConsultingCapabilityIconId, TablerIcon> = {
...
const { capabilities } = CLOUD_CONSULTING_SERVICES_PAGE;

export function CloudConsultingCapabilities() {
  return (
    <section className="w-full border-t border-[var(--border)] py-10 sm:py-11">
      <ContentContainer>
        <h2 className="mb-1.5 max-w-3xl text-balance font-sans text-5xl font-bold leading-[1.05] tracking-[-0.03em] text-[var(--text)] md:text-6xl">
          Six cloud capabilities one outcome <span className="text-[var(--red)]">infrastructure that works</span>
        </h2>
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {capabilities.items.map(({ title, body, icon }) => {
