...
const { faq } = CLOUD_CONSULTING_SERVICES_PAGE;

export function CloudConsultingFaq() {
  const baseId = useId();
    ...
    <section className="w-full border-t border-[var(--border)] py-10 sm:py-11">
      <ContentContainer>
        <h2 className="mb-5 max-w-3xl text-balance font-sans text-5xl font-bold leading-[1.05] tracking-[-0.03em] text-[var(--text)] md:text-6xl">
          Things teams ask <span className="text-[var(--red)]">before starting</span>
        </h2>
            ...
            return (
              <div key={q} className={isLast ? "" : "border-b border-[var(--border)]"}>
                <button
                  type="button"
                  id={buttonId}
                  className="flex w-full items-center justify-between gap-4 px-4 py-4 text-left transition-colors hover:bg-[var(--bg)]/50"
